...
//    }
    ...
    //modify power to 1 from 0.8 2/1/2020, need faster lift up or down when stack the block on platform
...
//        // Make sure the lift position >0 and < 4000 (around 11 bricks)
    ...
    public void startIntakeWheels() {
        intakeMotorLeft.setPower(0.8); //0.5
        intakeMotorRight.setPower(-0.8); //0.5
    ...
    //parameter for direction of conveyor : 1=reverse(outward),2=opposite(inward),3=stop
    public void reverseIntakeWheels(int conveyor){
        intakeMotorLeft.setPower(-0.8);  //-0.5
        intakeMotorRight.setPower(0.8); //0.5
        if (conveyor==1) {
    ...
    public void setIntakeDirection(IntakeDirection direction){
        if(direction == IntakeDirection.TAKE_IN){
            ...
            reverseIntakeWheels(1);
        } else if(direction == IntakeDirection.INTAKE_OUT_CONVEYOR_IN){
            reverseIntakeWheels(2);
        } else {
            reverseIntakeWheels(3);
        }
    }
    ...
    public void stopTape() {
        tapeDrive.setPosition(0.5);  //0.5 is 0 power to the motor
        //tapeDrive.setPosition(0.6);
    }
    ...
    public void initCapStonePosition(){
        capStoneServo.setPosition(profile.hardwareSpec.capStoneUp);
    }
    ...
    public enum IntakeDirection {TAKE_IN, STOP, EJECT_EXTRA,INTAKE_OUT_CONVEYOR_IN}
